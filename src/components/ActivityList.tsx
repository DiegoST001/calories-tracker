...
export default function ActivityList( { activities,  dispatch} : ActivityListProps  ){
    ...
    const categoryName = useMemo(() => (category : Activity['category']) => categories.map(item => item.id === category ? item.name : '') , [activities])
    ...
    return(
        <>
            ...
            activities.map((activity) => {
                return(
                    <div key={activity.id} className="px-5 py-10 bg-stone-100 mt-5 flex justify-between hover:scale-x-105 duration-150 shadow">
